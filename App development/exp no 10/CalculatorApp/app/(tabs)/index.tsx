...
// In your Calculator Screen file (e.g., one.tsx or index.tsx)

import firestore from '@react-native-firebase/firestore';

const saveCalculationToFirestore = async (expression: string, result: string) => {
  try {
    // 'history' is the name of our collection.
    // Firestore will create it automatically if it doesn't exist.
    await firestore().collection('history').add({
      expression: expression,
      result: result,
      createdAt: firestore.FieldValue.serverTimestamp(), // Use server timestamp for accurate ordering
    });
...
export default function CalculatorScreen() {
  ...
  const handleEqual = (isChainedOperation = false) => {
    ...
    if (!isChainedOperation) {
      ...
      saveCalculationToFirestore(expression, finalResultString);
      setOperator(null);
    }
