...
import { Link, useLocalSearchParams, useRouter } from 'expo-router'; // ✅ 1. IMPORT Link
...
export default function AnimeDetailScreen() {
  const router = useRouter(); // Hook to control navigation (e.g., go back)
  const { id } = useLocalSearchParams<{ id: string }>(); // Gets the [id] from the URL
  const ANIME_ID = Number(id); // Convert the ID from a string to a number
  ...
  useEffect(() => {
    ...
    fetchAllData();
  }, [ANIME_ID]); // Re-fetch if the ID changes
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView>
        <ImageBackground source={{ uri: details.images.jpg.large_image_url }} style={styles.headerImage} resizeMode="cover">
          ...
          {/* This button now uses the router to go back */}
...
const getStyles = (themeColors: typeof lightColors) => StyleSheet.create({
  ...
  backButton: { 
    position: 'absolute', 
    top: Platform.OS === "android" ? 10 : 50, // Adjusted for safe area
    left: 16, 
